...
(() => {
    ...
    const TIPOS_CONTENIDO = new Set(["image/jpeg", "image/png", "image/webp", "video/mp4", "video/webm"]);
    ...
    function iniciar() {
        ...
        cliente = window.supabase.createClient(window.REORGANICO_SUPABASE.url, window.REORGANICO_SUPABASE.anonKey, {
            auth: { storage: window.sessionStorage, storageKey: "reorganico-admin-auth-v1", persistSession: true, autoRefreshToken: true, detectSessionInUrl: true, flowType: "pkce" }
        });
        crearInput();
        agregarBotones();
        ...
        if (lista) {
            new MutationObserver(() => agregarBotones()).observe(lista, { childList: true, subtree: true });
    ...
    function configurarFormularioContenido() {
        ...
        formulario.addEventListener("submit", async (evento) => {
            if (destino.value === "carrusel" || destino.value === "blog") {
                evento.preventDefault();
                evento.stopImmediatePropagation();
                await subirContenidoMultiple(destino.value);
            }
        }, true);
    ...
    async function subirContenidoMultiple(destino) {
        ...
        if (!archivos.length) return mostrarMensajeContenido(mensaje, "Selecciona al menos una foto o video.", false);
        if (destino === "blog" && !titulo) return mostrarMensajeContenido(mensaje, "Escribe el título de la publicación.", false);
        if (destino === "blog" && !descripcion) return mostrarMensajeContenido(mensaje, "Escribe la descripción de la publicación.", false);
        ...
        if (!validos.length) return mostrarMensajeContenido(mensaje, destino === "blog" ? "Los archivos deben ser JPG, PNG, WEBP, MP4 o WEBM y pesar máximo 100 MB cada uno." : "Selecciona fotos JPG, PNG o WEBP de máximo 10 MB cada una.", false);
        ...
        try {
            const { data: ultimo, error: errorOrden } = await cliente.from("contenido_galeria").select("orden").eq("destino", destino).order("orden", { ascending: false }).limit(1).maybeSingle();
            ...
            for (const archivoActual of validos) {
                ...
                const { error: errorSubida } = await cliente.storage.from("galeria").upload(ruta, archivoActual, { cacheControl: "31536000", upsert: false, contentType: archivoActual.type });
                if (errorSubida) {
                ...
                const { error: errorRegistro } = await cliente.from("contenido_galeria").insert({
                    tipo,
            ...
            if (subidas > 0) {
                mostrarMensajeContenido(mensaje, `${subidas} archivo${subidas === 1 ? "" : "s"} publicado${subidas === 1 ? "" : "s"} correctamente en ${destino === "blog" ? "Nuestro Blog" : "el carrusel"}.`, true);
                archivo.value = "";
    ...
    async function subirFotos(archivos, id) {
        ...
        const validos = archivos.filter((archivo) => TIPOS_FOTO.has(archivo.type) && archivo.size <= MAX_FOTO);
        if (!validos.length) {
        ...
        for (const archivo of validos) {
            ...
            const { error } = await cliente.storage.from(BUCKET).upload(ruta, archivo, { cacheControl: "31536000", upsert: false, contentType: archivo.type });
            if (!error) subidas += 1;
            else console.error("No se pudo subir la foto:", error);
        }
        if (subidas > 0) {
            const { data: producto } = await cliente.from("productos").select("imagen_admin_path").eq("id", id).maybeSingle();
            if (!producto?.imagen_admin_path) {
                const { data: fotos } = await cliente.storage.from(BUCKET).list(id, { limit: 100, sortBy: { column: "name", order: "asc" } });
                const primera = (fotos || []).find((foto) => foto.name);
                if (primera) await cliente.from("productos").update({ imagen_admin_path: `${id}/${primera.name}` }).eq("id", id);
            }
        }
        ocupado = false;
        if (boton) {
        ...
        if (subidas === validos.length) alert(`${subidas} foto${subidas === 1 ? " agregada" : "s agregadas"} correctamente al producto.`);
        else alert(`${subidas} foto${subidas === 1 ? " agregada" : "s agregadas"}. ${validos.length - subidas} no se pudo subir.`);
