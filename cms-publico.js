...
(function () {
    ...
    function renderizarCarrusel(items) {
        ...
        const contenidos = [...existentes.map((html) => ({ html })), ...items.map((item) => ({ html: crearMedia(item) }))];
...
(() => {
    ...
    const nombreProducto = (tarjeta) => tarjeta?.querySelector("h3,h2,.producto-nombre,[data-nombre]")?.textContent?.trim() || "Producto Re Orgánico";
    ...
    function crearModal() {
        ...
        modal.querySelector(".compartir-cerrar").addEventListener("click", cerrar);
        ...
        modal.addEventListener("click", async (e) => {
            ...
            try {
                ...
                if (accion === "nativo") {
                    if (navigator.share) await navigator.share({ title: nombre, text: texto, url: enlace });
                    else {
                        await copiar(enlace);
                        alert("El enlace fue copiado porque tu navegador no tiene compartir nativo.");
                    }
                    return;
                }
                ...
                const destinos = {
                    ...
                    email: `mailto:?subject=${encodeURIComponent(nombre)}&body=${encodeURIComponent(texto + "\n\n" + enlace)}`
                };
    ...
    function instalar() {
        document.querySelectorAll("article.producto[data-id]").forEach((tarjeta) => {
            ...
            boton.addEventListener("click", (e) => {
                e.preventDefault();
                e.stopPropagation();
                abrirCompartir(id, tarjeta);
            });
            (tarjeta.querySelector(".producto-acciones") || tarjeta).appendChild(boton);
        });
    }

    const estilo = document.createElement("style");
    estilo.textContent = `.boton-compartir-producto{display:inline-flex;align-items:center;justify-content:center;gap:6px;margin-top:8px;padding:9px 13px;border:1px solid #6f9b73;border-radius:999px;background:#f3f8f1;color:#2f6840;font-size:13px;font-weight:800;cursor:pointer}.boton-compartir-producto:hover{background:#e5f0e3}.boton-compartir-producto:focus-visible{outline:3px solid rgba(47,104,64,.28);outline-offset:2px}.modal-compartir-producto{position:fixed;inset:0;z-index:99999;display:none;align-items:flex-start;justify-content:center;padding:8vh 16px;background:rgba(14,35,23,.58);box-sizing:border-box;overflow:auto}.modal-compartir-producto.activo{display:flex}.compartir-caja{position:relative;width:min(94vw,520px);padding:28px;border-radius:22px;background:#fff;box-shadow:0 24px 70px rgba(0,0,0,.25);box-sizing:border-box}.compartir-cerrar{position:absolute;top:12px;right:14px;width:38px;height:38px;border:0;border-radius:50%;background:#edf4ed;color:#205b38;font-size:25px;cursor:pointer}.compartir-caja span{color:#438052;font-size:12px;font-weight:900;text-transform:uppercase}.compartir-caja h2{margin:7px 42px 18px 0;color:#173c27;font-size:22px}.compartir-opciones{display:grid;grid-template-columns:repeat(2,1fr);gap:9px}.compartir-opciones button,.compartir-nativo{min-height:44px;border:1px solid #d7e3d8;border-radius:12px;background:#f7faf7;color:#205b38;font-weight:800;cursor:pointer;padding:10px 12px}.compartir-opciones button:hover,.compartir-nativo:hover{background:#e9f2e9}.compartir-nativo{width:100%;margin-top:10px}@media(max-width:600px){.modal-compartir-producto{padding:4vh 10px}.compartir-caja{width:100%;padding:24px 18px}.compartir-opciones{grid-template-columns:1fr 1fr}}`;
